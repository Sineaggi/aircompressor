...
public final class SnappyRawDecompressor
{
    ...
    public static int getUncompressedLength(ArrayUtil compressed, long compressedAddress, long compressedLimit)
    {
            ...
            final long outputLimit)
    {
        ...
        while (input < inputLimit) {
            ...
            if (input + SIZE_OF_INT < inputLimit) {
                trailer = inputBase.getInt(input) & wordmask[trailerBytes];
            }
            else {
                ...
                switch (trailerBytes) {
                    case 4:
                        trailer = (inputBase.getByte(input + 3) & 0xff) << 24;
                    case 3:
                        trailer |= (inputBase.getByte(input + 2) & 0xff) << 16;
                    case 2:
                        trailer |= (inputBase.getByte(input + 1) & 0xff) << 8;
                    case 1:
                        trailer |= (inputBase.getByte(input) & 0xff);
                }
            }
            if (trailer < 0) {
                throw new MalformedInputException(input - inputOffset);
     ...
     */
    static int[] readUncompressedLength(ArrayUtil compressed, long compressedAddress, long compressedLimit)
    {
        int result;
        int bytesRead = 0;
        {
    ...
    private static int getUnsignedByteSafe(ArrayUtil base, long address, long limit)
    {
        if (address >= limit) {
            throw new MalformedInputException(limit - address, "Input is truncated");
        }
        return base.getByte(address) & 0xFF;
    }
}
